<template>
    ...
    <div class="bg_table_3">
      ...
      >
        ...
        <el-table-column v-if="checkedColumns.includes('Cancer 1')" prop="name.x" label="Cancer 1" min-width="120px" align="center" />
        <el-table-column v-if="checkedColumns.includes('Cancer 2')" prop="name.y" label="Cancer 2" min-width="120px" align="center" />
        <el-table-column v-if="checkedColumns.includes('Loci region')" label="Loci region" min-width="240px" align="center" >
          <template v-slot="slotProps">
            {{ slotProps.row.chr }}:{{ slotProps.row.start }}-{{ slotProps.row.end }}
          </template>
        </el-table-column>
        <el-table-column v-if="checkedColumns.includes('nPleioSNPs')" prop="num_pleio" min-width="100px" align="center" >
          <template v-slot:header>
            <span>nPleioSNPs</span>
            <el-tooltip
              content="Number of pleiotropic SNPs in this region"
              placement="top-start"
              effect="light"
            >
              <el-icon class="icon">
        ...
        <el-table-column v-if="checkedColumns.includes('IndSigSNPs')" prop="IndSigSNPs" min-width="120px" align="center" >
          <template v-slot="scope">
            <span>nIndSigSNPs</span>
            <el-tooltip
              content="Independent significant SNPs (r2 < 0.6)"
              placement="top-start"
              effect="light"
            >
              <el-icon class="icon">
            ...
            <span v-for="(snp, index) in splitSNPs(scope.row.IndSigSNPs)" :key="index">
              <a
                :href="'http://localhost:8081/cancerdb/search_pleio_snp?snp=' + snp"
                target="_blank"
        ...
        <el-table-column v-if="checkedColumns.includes('LeadSNPs')" prop="LeadSNPs" min-width="120px" align="center" >
          <template v-slot="scope">
            <span>LeadSNPs</span>
            <el-tooltip
              content="r2 < 0.1"
            ...
            <span v-for="(snp, index) in splitSNPs(scope.row.LeadSNPs)" :key="index">
              <a
                :href="'http://localhost:8081/cancerdb/search_pleio_snp?snp=' + snp"
                target="_blank"
                style="margin-right: 5px"
        ...
        <el-table-column v-if="checkedColumns.includes('Cytoband')" prop="Cytoband" label="Cytoband" min-width="120px" align="center" />
      </el-table>
    </div>
...
const columnOptions = ['Cancer 1', "Cancer 2",'Loci region','nPleioSNPs', 'IndSigSNPs', 'LeadSNPs', 'Cytoband'];
const cancerTypes = [
...
mounted() {
  ...
  if(this.query){
    ...
    this.loading = true,
    axios.get("/api/cancerdb/pleio_fuma").then((response) => {
...
methods: {
  splitSNPs(inputSplitSnp) {
    return inputSplitSnp ? inputSplitSnp.split(';') : []
  },
  searchsnp_eg() {
