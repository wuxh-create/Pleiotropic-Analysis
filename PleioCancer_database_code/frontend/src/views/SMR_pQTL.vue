<template>
  ...
  <div class="search">
    <el-row class="searchContent" :gutter="20">
      ...
      <el-col :span="7">
        <span class="geneInputBox">Protein (Gene symbol)</span>
        <el-input v-model="searchProtein" clearable style="position:relative; top:5px" />
      </el-col>
      <el-col :span="7">
    ...
    <el-row class="searchExample" :gutter="20">
      ...
      <el-col :span="8">
        <el-button
          ...
          style="font-size: 16px;"
          >1:149895209-159895209</el-button
        >
      </el-col>
    </el-row>
  </div>
  ...
  <div class="table_display">
    ...
    <div>
      <div id="chart_heatmap" ref="chart_heatmap" :style="chartStyle"
      style="transition: all 0.3s ease-in-out;"></div>
    </div>
  ...
  <div class="bg_table_3">
      ...
      <el-table-column v-if="checkedColumns.includes('Protein')" prop="Gene" label="Protein" min-width="120px" align="center" >
        <template v-slot="scope">
          <a :href="'http://localhost:8081/cancerdb/search_gene?gene=' + scope.row.Gene" target="_blank" class="link-snpid">
            {{ scope.row.Gene }}
          </a>
        </template>
      </el-table-column>
      <el-table-column v-if="checkedColumns.includes('Protein position')" label="Protein position" min-width="120px" align="center">
      ...
      <el-table-column v-if="checkedColumns.includes('pQTL_beta')" prop="b_eQTL" label="pQTL_beta" min-width="120px" align="center" sortable="custom"/>
      <el-table-column v-if="checkedColumns.includes('pQTL_SE')" prop="se_eQTL" label="pQTL_SE" min-width="120px" align="center" sortable="custom"/>
      <el-table-column v-if="checkedColumns.includes('pQTL_P')" prop="p_eQTL" label="pQTL_P" min-width="120px" align="center" sortable="custom"/>
  ...
  <div class="fig_description">
      ...
      SMR/pQTL/GWAS figures in protein and its ± 1Mb flanking region based on the selected protein 
      <span :style="{color: selectedProtein_symbol ? '#ffd04b' : 'white'}">{{ selectedProtein_symbol }}</span> 
      and cancer 
      <span :style="{color: selectedCancer ? '#ffd04b' : 'white'}">{{ selectedCancer }}</span>
      </span>
  </div>
  <div class="plot" v-loading="loading_echart">
      <div id="chart_pQTL" ref="chart_pQTL" style="width: 100%; height: 800px;"></div>
...
const columnOptions = ['Protein', "Protein position", 'Cancer',"SMR_beta","SMR_SE","SMR_P","SMR_P_multi","SMR_FDR","HEIDI_P","HEIDI_nsnp", 'Top SNP', 'SNP position', "EA/OA", "EAF", "pQTL_beta","pQTL_SE","pQTL_P","GWAS_beta","GWAS_SE","GWAS_P"];

export default {
  name: "pQTL_SMR",
  data() {
    const cancerTypes = ['Basal cell carcinoma', 'Breast cancer', 'Colorectal cancer','Kidney cancer, except renal pelvis', 
    'Ovarian cancer', 'Prostate cancer', 'Skin melanoma', 'Squamous cell carcinoma', 'Thyroid cancer'];
    ...
    return {
      ...
      selectedProtein: '', // 动态蛋白ID，有的一个ID对应多个symbol
      selectedProtein_symbol:'',
      selectedCancer: '', // 动态癌症
  ...
  mounted() {
    ...
    if(this.query){
      ...
      this.cancerType = "",
      this.loading = true,
      axios.get("/api/cancerdb/SMR_pQTL").then((response) => {
  ...
  methods: {
    ...
    searchprotein_eg() {
      // 获取按钮文本内容并赋值给searchSNPs
      const buttonText = event.target.textContent.trim();
      this.searchProtein = buttonText; // 更新输入框内容
      this.searchRegion = "";
    },
    ...
    selectRow(row, value) {
      ...
      if (this.currentSelectedRow === row) {
        ...
        row.check = value;
        this.selectedProtein = row.probeID;  // 更新选中的基因
        this.selectedProtein_symbol = row.Gene;
    ...
    exportToExcel() {
      ...
      link.setAttribute('download', 'SMR_pQTL_FDR0.05.csv');
    ...
    sendFigData() {
      ...
      axios.post("/api/cancerdb/SMR_pQTL_fig", {protein: this.selectedProtein, cancer: this.selectedCancer})
        .then(response => {
    ...
    plotManhattanWithChartJS(input_data) {
      this.chart_pQTL = markRaw(echarts.init(this.$refs.chart_pQTL,null, { renderer : 'svg' })); // 渲染为svg，防止放大模糊

      // 提取坐标和 -log10(P)
      const pQTLData_sig = input_data.pQTL_result_sig.map(item => {
        return [item.BP, item.log10P];
      });
      const pQTLData_notsig = input_data.pQTL_result_notsig.map(item => {
        return [item.BP, item.log10P];
      });
      ...
      const xAxisConfig = {
        type: "value",
        min: input_data.protein_start,
        max: input_data.protein_end,
        splitNumber: 10,
      ...
      const option = {
        tooltip: {
          ...
          formatter: function(params) {
            ...
            if (seriesIndex === 0) {
              ...
              tooltipContent = `<b>Protein:</b> ${data_tooltip.Gene}<br/>
                                <b>Protein position:</b> ${data_tooltip.ProbeChr}:${data_tooltip.Probe_bp}<br/>
              ...
              const data_tooltip = input_data.pQTL_result_sig[params.dataIndex];
              ...
              const data_tooltip = input_data.pQTL_result_notsig[params.dataIndex];
              tooltipContent = `<b>SNP:</b> ${data_tooltip.SNP}<br/>
                                <b>SNP position:</b> ${data_tooltip.Chr}:${data_tooltip.BP}<br/>
                                <b>EA/OA:</b> ${data_tooltip.A1}/${data_tooltip.A2}<br/>
                                <b>EAF:</b> ${data_tooltip.Freq}<br/>
        ...
        yAxis: [
          ...
          { ...yAxisConfig, name: "-log10(pQTL_P)", gridIndex: 1 },
    ...
    queryData() {
      ...
      // 检索条件返回后端进行查询
      this.loading = true
      axios.post("/api/cancerdb/SMR_pQTL",{cancerType:this.cancerType,searchRegion:this.searchRegion,searchProtein:this.searchProtein})
        .then((res) => {
