...
smr_meqtl_bp = Blueprint("smr_meqtl", __name__)
...
@smr_meqtl_bp.route("/SMR_meQTL", methods=["GET", "POST"])
def smr_meqtl():
    listx = []
    if request.is_json:
        ...
        searchMethy = filterForm['searchMethy']
        ...
        if filterForm['searchRegion']=="" or (len(searchRegion)==3 and all(searchRegion) and int(searchRegion[1])<int(searchRegion[2])):
            ...
            if searchMethy:  # 如果基因参数不为空
                query['probeID'] = searchMethy
            ...
            # 执行MongoDB查询
            for x in SMR_meQTL.find(query, {"_id": 0}):
                listx.append(x)
    
        else:
            return "errorID"
    elif request.method == "GET":
        methy = request.args.get("searchMethy")
        if methy:
            listx_meQTL = []
            listx_smr_meQTL = []            
            listx_smr_meQTL_eqtl = []
            listx_meQTL = list(meQTL.find({"Probe": methy,"sig":"Yes"}, {"_id": 0}))
            listx_smr_meQTL = list(SMR_meQTL.find({"probeID": methy,"sig": "Yes"},{"_id": 0}))
            listx_smr_meQTL_eqtl = list(smr_meqtl_eqtl.find({"Expo_ID": methy},{"_id": 0}))
            if len(listx_smr_meQTL)>0:
                return {"meQTL":listx_meQTL, "smr_meQTL":listx_smr_meQTL,"smr_meQTL_eQTL":listx_smr_meQTL_eqtl}
            else:
                return {"meQTL":[], "smr_meQTL":[],"smr_meQTL_eQTL":[]}
        else:
            for x in SMR_meQTL.find({"sig":"Yes"}, {"_id": 0}):
                listx.append(x)
    return jsonify(listx)

@smr_meqtl_bp.route("/SMR_meQTL_fig", methods=["GET", "POST"])
def smr_meQTL_fig():
    if request.is_json:
        data_frontEnd = request.get_json()
        methy = data_frontEnd["methy"]
        cancer = data_frontEnd["cancer"]

        meQTL_result = list(meQTL.find({"Probe": methy}, {"_id": 0}))
        meQTL_result_notsig = [x for x in meQTL_result if float(x['p']) >= 5e-8]
        meQTL_result_sig = [x for x in meQTL_result if float(x['p']) < 5e-8]

        
        methy_start = meQTL_result[1]["Probe_bp"] - 1000000
        methy_end = meQTL_result[1]["Probe_bp"] + 1000000
        GWAS_result = list(GWAS.find(
            {"cancer": cancer, "hg19chr": meQTL_result[1]["Probe_Chr"], "bp": {"$gte": methy_start, "$lte": methy_end}},
            {"_id": 0}
        ))
        ...
        SMR_result = list(SMR_meQTL.find(
            {"cancer": cancer, "ProbeChr": meQTL_result[1]["Probe_Chr"], "Probe_bp": {"$gte": methy_start, "$lte": methy_end}},
            {"_id": 0}
        ))
        ...
        return jsonify({"meQTL_result_sig": meQTL_result_sig, "meQTL_result_notsig": meQTL_result_notsig, 
