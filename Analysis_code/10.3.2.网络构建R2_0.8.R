...
for(cancer in list.files()){
  load(cancer)
  load(paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/sft/", cancer))
  ...
  net = blockwiseModules(filtered_expr, #处理好的表达矩阵
                         ...
                         saveTOMFileBase = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/fpkmTOM/", gsub(".Rda","",cancer)),
                         verbose = 3)#0，不反回任何信息，＞0返回计算过程
  # 保存网络构建结果
  save(net, file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/one_step_net/", cancer))
  
  # # 加载网络构建结果
  # load(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/one_step_net/", cancer))
  # 打开绘图窗口
  pdf(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/moduleCluster/", gsub(".Rda", "", cancer), ".pdf"), width = 12, height = 9);
  # 将标签转化为颜色
  ...
  # 加载TOM矩阵
  load(paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/fpkmTOM/", gsub(".Rda","",cancer), "-block.1.RData"))
  # 网络特征向量
  MEs = moduleEigengenes(filtered_expr, mergedColors)$eigengenes
  # 对特征向量排序
  MEs = orderMEs(MEs)
  # 可视化模块间的相关性
  pdf(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/moduleCor/", gsub(".Rda", "", cancer), ".pdf"), width = 5, height = 7.5);
  par(cex = 0.9)
  ...
  # pdf(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/TOMplot/", gsub(".Rda", "", cancer), ".pdf"), width = 5, height = 5);
  # # 美化图形的设置
  ...
  png(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.8/TOMplot/", gsub(".Rda", "", cancer), ".png"), width = 1000, height = 1000);
  # 美化图形的设置
  plotDiss = dissTOM^7
  diag(plotDiss) = NA
  myheatcol = colorpanel(250,'red',"orange",'lemonchiffon')
