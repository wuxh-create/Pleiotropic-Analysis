...
for(cancer in list.files()){
  ...
  net = blockwiseModules(filtered_expr, #处理好的表达矩阵
                         ...
                         saveTOMFileBase = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/fpkmTOM/", gsub(".Rda","",cancer)),
                         verbose = 3)#0，不反回任何信息，＞0返回计算过程
  # 保存网络构建结果
  save(net, file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/one_step_net/", cancer))
  
  # # 加载网络构建结果
  # load(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/one_step_net/", cancer))
  # 打开绘图窗口
  pdf(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/moduleCluster/", gsub(".Rda", "", cancer), ".pdf"), width = 12, height = 9);
  # 将标签转化为颜色
  ...
  # 加载TOM矩阵
  load(paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/fpkmTOM/", gsub(".Rda","",cancer), "-block.1.RData"))
  # 网络特征向量
  MEs = moduleEigengenes(filtered_expr, mergedColors)$eigengenes
  # 对特征向量排序
  MEs = orderMEs(MEs)
  # 可视化模块间的相关性
  pdf(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/moduleCor/", gsub(".Rda", "", cancer), ".pdf"), width = 5, height = 7.5);
  par(cex = 0.9)
  ...
  # pdf(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/TOMplot/", gsub(".Rda", "", cancer), ".pdf"), width = 5, height = 5);
  # # 美化图形的设置
  ...
  png(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/TOMplot/", gsub(".Rda", "", cancer), ".png"), width = 1000, height = 1000);
  # 美化图形的设置
  plotDiss = dissTOM^7
  diag(plotDiss) = NA
  myheatcol = colorpanel(250,'red',"orange",'lemonchiffon')
...
m = matrix(ncol = 5,nrow=13)
cancers = list.files()
for(i in 1:length(cancers)){
  cancer = cancers[i]
  m[i,1] = gsub(".Rda","",cancer)
  
  load(cancer)
  mergedColors = labels2colors(net$colors)
  counts = table(mergedColors)
  c_without_grey = counts[names(counts)!="grey"]
  
  m[i,2] = length(c_without_grey) # 统计模块数目
  m[i,3] = min(c_without_grey) # 模块中基因最少的数目
  m[i,4] = max(c_without_grey)
  m[i,5] = counts[names(counts)=="grey"] # 灰色模块基因数
}
m = as.data.frame(m)
colnames(m) = c("cancer","module_num","min_module","max_module","grey_num")
...
# for(cancer in list.files())
  ...
  MEs = moduleEigengenes(filtered_expr, mergedColors)$eigengenes # 模块特征向量
  # 计算基因的 KME
  KME <- as.data.frame(cor(filtered_expr, MEs, use = "p"))
  # 筛选指定模块中的基因
  # 有2901个基因在turquoise中
  selected = names(which.max(table(mergedColors)))
  KME_filtered = KME[mergedColors==selected,]
  
  library(ggplot2)
  library(tidyr)
  df_long <- pivot_longer(KME_filtered, cols = everything(), names_to = "variable", values_to = "value")
  
  # 绘制箱线图
  ggplot(df_long, aes(x = variable, y = value)) +
    geom_boxplot(fill = "lightblue") +
    labs(title = "各变量的箱线图", x = "变量", y = "值") +
    theme_minimal()
}
