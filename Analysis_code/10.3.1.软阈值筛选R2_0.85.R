...
for(cancer in list.files()){
  ...
  # 2.1 构建自动化网络和检测模块
  # 选择软阈值
  pdf(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/soft_thresh/", gsub(".Rda", "", cancer), ".pdf"), height = 5 ,width = 8)
  # sizeGrWindow(9, 5)
  par(mfrow = c(1,2))
  ...
  abline(h=0.85,col="red")  #查看位于0.9以上的点，可以改变高度值   
  # 平均连接度
  ...
  save(sft, file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/sft/", cancer))
  ...
  pdf(file = paste0("/home/yanyq/share_genetics/result/WGCNA/r2_0.85/scale_free/", gsub(".Rda", "", cancer), ".pdf"), height = 5 ,width = 8)
  par(mfrow = c(1,2))
  hist(k)#直方图
